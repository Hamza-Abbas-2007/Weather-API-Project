...
class todo (db.Model):
    ...
    Humidity = db.Column(db.String(200), nullable = True)
...
def Get_Humidity(city):
    cache_key = f'Humidity_{city}'
    
    if cache_key in cache:
        print("Cache hit: Returning cached Humidity")
        return cache[cache_key]
    ...
    if response.status_code == 200:
        ...
        cache.set(cache_key, humidity , expire=60)
        print(f"Cached Humidity successfully for {city}")
        return humidity
...
def index():
    ...
    cachedHumidity = Get_Humidity(city) 
    if request.method == 'POST':
    
        new_entry = todo(content=str(cachedtemp), city_name=str(city), Humidity = str(cachedHumidity))
        try:
